...
-   Better file structure, removed some complexity from the format
-   Introduction of pepper bytes: Added security of 512 bits
-   Better reorder hashing algorithms by general security (by "security" I mean how well it can be used to check integrity of data)
...
pDB is a little-endian format, all binary data must be little endian.
...
-   `uint16_t version` (2 little endian bytes, `<H`) -- The database version starting from 0.
...
-   `uint8_t hash_id` (1 little endian byte, `<B`) -- The hashing algorithm used for hashing across the database. This value is hardly mutable, as the _whole_ database would need to be rehashed (and possible revalidated) for it to work.
...
-   `uint16_t rc4_crypto_passes` (2 little endian bytes, `<H`) -- The count of RC4 encryption passes to do every time insecure encryption is initiated.
...
-   `uint16_t chacha20_crypto_passes` (2 little endian bytes, `<H`) -- The count of custom ChaCha20 encryption passes to do every time secure encryption is initiated.
-   `uint8_t metadata_hash[hash_size]` (`hash_size` little endian bytes, `<{hash_size}B`) -- Using the configured hashing algorithm, the encrypted metadata of the database is algorithmically hashed and the hash stored in this section for future validation.
-   `uint64_t metadata_size` (4 little endian bytes, `<L`) -- The size of the metadata blob following it.
-   `uint8_t metadata[metadata_size]` (`metadata_size` little endian bytes, `<{metadata_size}B`) -- The metadata of the database. This section may be cleared at any moment and you should not use it to store anything sensitive and persistent.
...
This section describes the loading, authentication, and decryption workflow for pDBv1:
...
Of course, it may differ, but this is an example workflow of how generic processes may work. A pDB client is not
be considered a pDB client if it does not implement at least the described workflows partially.
...
-   Secure hashing
    ...
    -   Run PBKDF2 with the HMAC pseudorandom function
